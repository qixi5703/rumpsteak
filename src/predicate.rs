...
pub struct GTnVar<L, V: Ord, const LHS: char, const RHS: char> {
    _p: PhantomData<(V, L)>,
}

impl<L, V: Ord, const LHS: char, const RHS: char> Default for GTnVar<L, V, LHS, RHS> {
    fn default() -> Self {
        Self { _p: PhantomData }
    }
}

impl<L, V: Ord, const LHS: char, const RHS: char> Predicate for GTnVar<L, V, LHS, RHS>
where
    V: std::cmp::Ord
{
    type Name = char;
